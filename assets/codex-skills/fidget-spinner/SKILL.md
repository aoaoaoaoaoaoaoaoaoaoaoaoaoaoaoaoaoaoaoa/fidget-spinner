...
- `experiment.close` only for an already-open experiment and only when you have a verdict and rationale. This is the canonical way to put material hypothesis-driven evidence into the operator-visible record. Non-scuffed closes require at least one measured frontier KPI, with exactly one designated primary. Every other KPI is optional. Report another KPI only when a meaningfully comparable value was actually measured; never copy, inherit, or impute unrelated KPI values merely to complete a frontier-wide row. They also require a clean git worktree and record `HEAD` automatically, anchoring to `command.working_directory` when provided, so make a fast commit in the actual implementation worktree first. A scuffed close is the rare exception: use it when no meaningful KPI exists because the experiment itself was invalid. Attach `analysis` only when the result needs interpretation beyond the rationale. If this is the last open experiment for the owning hypothesis, explicitly pass `keep_hypothesis_on_worklist=true` to keep pursuing it or `false` to shelve it
- `experiment.scuff` only as a rare break-glass correction for an already-closed experiment whose setup or recorded value was invalid. Scuffed means the experiment itself was fucked up: incoherent conditions, accidental experiment opening, missing meaningful KPI, duplicate/dummy point, or another procedural failure. It is preserved for audit but ignored by plots and rankings. Do not use scuffed for valid negative evidence
...
- `kpi.list` or `metric.keys --scope kpi` before guessing which frontier metrics define the real hill; `kpi.list` includes named reference lines
...
6. Close the experiment with `experiment.close`, including conditions, one primary frontier KPI unless scuffed, any supporting metrics actually observed, verdict, rationale, optional analysis, and `keep_hypothesis_on_worklist` when this closure leaves the owning hypothesis with no open experiments. Spinner will reject a dirty worktree and store the closing commit hash automatically for non-scuffed closes.
7. Use `verdict=rejected` when the experiment produced valid values that invalidate the hypothesis or should not enter the final code. Use `verdict=scuffed`, or retroactively `experiment.scuff`, only when the experiment procedure itself was invalid and a meaningful KPI could not be obtained; scuffed results are basically “delete this point, but remember that we tried.”
...
6. Treat metric keys as project-level registry entries and conditions as the
   ...
   timeout, hardware, or dataset; measured outcomes belong in metrics. They do
   not excuse a change in what a metric means or the comparability of its
   values. Do not encode scenario context or Hungarian unit notation into the
   metric key: prefer
